var _ = require( 'underscore' ),
  ...
  http = require( 'http' ),
  ircbot = require( './bot' ),
  SlackMsgDecoder = require( './slackmsgdecoder' ).SlackMsgDecoder,
...
Slack2IRC.prototype._requestHandler = function( req, res ) {
  req.on( 'data', function( data ) {
    ...
    if ( payload.token == this.config.outcome_token && payload.user_name != 'slackbot' ) {
...
Slack2IRC.prototype._decodeMessage = function( text ) {
  ...
  .decodeChannel().decodeUser().decodeURL().decodeAngel();
};
