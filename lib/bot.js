...
Bot.prototype._handleErrors = function() {
  ...
  this.client.addListener('error', function(message) {
    ...
    var errorMessage = self.mapPronouns(message.args[2]);
    self.systemSpeak(channel, 'I don\'t feel so well because ' + errorMessage);
  });
};
...
Bot.prototype._trackUsers = function() {
  ...
  this.client.addListener('names', function(channel, nicks) {
    ...
    self.systemSpeak(channel, 'I\'m all over you slackers');
  });
  ...
  this.client.addListener('join', function(channel, nick, whois) {
    if (whois.user === myusername) {
      ...
      self.systemSpeak(channel, 'i\'m watching you slacker @' +
  ...
  this.client.addListener('nick', function(oldNick, newNick, channels) {
    ...
    self._usermap.nicks[newNick] = self._usermap.nicks[oldNick];
    delete self._usermap.nicks[oldNick];
    channels.forEach(function(channel) {
      self.systemSpeak(channel, 'don\'t think you can hide slacker @' +
             self._usermap.nicks[newNick]);
    });
  });
};
...
Bot.prototype.sentToSlack = function(from, to, message) {
  var
  avatar,
  avatars = this.config.slackAvatars,
  username = this._usermap.nicks[from] || from,
  hasAvatar = typeof avatars[username] !== 'undefined';

  avatar = hasAvatar ? avatars[username] : this.config.icon_url;

  if(!message.match(/.+url.*:\ \[.+\]/)) {
    this.slacker.send('chat.postMessage', {
      channel: this.config.ircChannels[0],
      text: this.prepareMessage(message, this._usermap.nicks),
      username: username,
      icon_url: avatar,
...
Bot.prototype.prepareMessage = function(message, users) {
  ...
  return IrcMsgEncoder(message)
    .setChanelList(this.config.channels).setUserList(this.config.users)
    .encodeUser().toString();
};
